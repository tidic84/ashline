...
@export var HEAD_ANGLE_RANGE:Vector2 = Vector2(20, 160)
...
func _input(event):
    if event is InputEventMouseMotion:
        ...
        #cam_rot_spd = Vector2(0.2, 0.2) * event.relative
...
func _process_control(delta):
    if not rotation_helper or not active:
        return
    ...
    var cam_spd =  abs(cam_rot_spd.x * delta * 5.0) #* 0.02
    ...
    var new_target_rotation_helper_basis = target_rotation_helper_basis.rotated(
        Vector3.RIGHT,
        -deg_to_rad(new_rot.x *  MOUSE_SENSITIVITY)
    )
    new_target_rotation_helper_basis = new_target_rotation_helper_basis.rotated(
        Vector3.RIGHT,
        deg_to_rad(joy_look.y)
    )

    # limit head up-down rotation
    var dest_up_down_angle = rad_to_deg(new_target_rotation_helper_basis.z.angle_to(Vector3.UP))
    var dest_up_down_angle_valid = new_target_rotation_helper_basis.z.cross(Vector3.UP).x < 0

    if (
        dest_up_down_angle > HEAD_ANGLE_RANGE.x
        and dest_up_down_angle < HEAD_ANGLE_RANGE.y
        and dest_up_down_angle_valid
    ):
        target_rotation_helper_basis = new_target_rotation_helper_basis

...
@export_node_path("UP_Killable") var killable_behaviour_path:NodePath = NodePath("")
...
func _ready():
    ...
    if killable_behaviour:
        killable_behaviour.killed.connect(_killed)
        killable_behaviour.respawned.connect(_respawned)

...
@export_range(0.01, 1.0) var FRICTION_IN_AIR = 0.5
...
func _ready():
    ...
    player.controllable_changed.connect(_stop_walking)

    _land_foot_timer.wait_time = 0.2
    _land_foot_timer.one_shot = true
    _land_foot_timer.timeout.connect(_on_land_foot_timer)
...
func _process_control(delta):
    if _first_frame and rotation_helper:
        _first_frame = false
        # FIXME: order of the initialization matters. Ho
    ...
    if enable_walk or enable_run and _is_running:
        input_movement_vector.y += Input.get_action_strength(ACTION_FORWARD)-Input.get_action_strength(ACTION_BACKWARD)
        input_movement_vector.x += Input.get_action_strength(ACTION_RIGHT)-Input.get_action_strength(ACTION_LEFT)
    ...
    if enable_jump and Input.is_action_just_pressed(ACTION_JUMP):
        if on_floor or jump_in_air: # and not fly_mode):
            jump.emit()
            jumped = true
            player.velocity.y = JUMP_SPEED_RUN if _is_running else JUMP_SPEED

    if enable_crouch and Input.is_action_just_pressed(ACTION_CROUCH):
        _set_crouch(not _crouch)

    _is_running = enable_run and Input.is_action_pressed(ACTION_RUN)
    _is_running = not _is_running if enable_run and ALWAYS_RUN else _is_running
...
func _process_movement(delta):
    ...
    if on_floor:
        ...
        if is_falling:
            is_falling = false
            land.emit(player.velocity) # FIMXE: should be contact velocity, not last known
            #footstep.emit(0)
            ...
            #    fall_damage.emit(hit_vel)  # must be before hit calc to properly handle is_alive flag
        ...
        if not is_falling:
            ...
            var _air_vel_factor = 1.0 -(FRICTION_IN_AIR * delta)
            velocity = Vector3(velocity.x * _air_vel_factor , velocity.y, velocity.z * _air_vel_factor)
...
func _process_bobbing(delta):
    ...
    if on_floor:
        ...
        if velocity_length < 0.2:
            ...
            current_bob_transform.origin += Vector3(
                        sin(_bob_stay)*BOB_STAY_FACTOR+randf_range(-1, 1)*BOB_STAY_SHAKINESS,
                        cos(_bob_stay)*BOB_STAY_FACTOR * 0.5,
                        0

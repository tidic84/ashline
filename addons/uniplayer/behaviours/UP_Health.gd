...
## If [member killable_behaviour_path] is set, a [method UP_Killable.kill]
...
## Path to [UP_Killable] node. If set, the player's character will be killed when
## health drops to 0 or below.
@export_node_path("UP_Killable") var killable_path:NodePath = NodePath("")

...
## A list of visual abilities, which will be called in [method _process_physics]
## only when [member controllable] is set to true.
## Abilities can override [method UP_VisualAbility._process_control()].
##
## [i]Visual abilities will be unnecesary when traits will be
## available.[/i/
var visual_abilities:Array[UP_VisualAbility] = []
...
## Register an ability in the [member visual_abilities] list.
## Ability can override [member UP_VisualAbility._process_control].
func register_visual_ability(ability:UP_VisualAbility):
    visual_abilities.append(ability)
...
func _process_movement(delta:float):
    if controllable:
        ...
        for ability in visual_abilities:
            ability._process_control(delta)
